...
int main(int argc, char *argv[])
{
	...
	while(1){
		printTimeStamp(); cout<<"Please enter the action (get/put/del): ";
		bzero(message,256);
		cin>>message;
		...
		if(strcmp(message,"get") == 0){
			bzero(message,256);
			printTimeStamp(); cout<<"Enter the key to get\n";
			cin>>message;
			if(write(socketfd,message,strlen(message)) < 0){
				printTimeStamp(); cout<<"error writing to socket\n";
				return 0;
			...
			bzero(message,256);
			printTimeStamp(); cout<<"Enter the key to be inserted\n";
			cin>>message;
			if(write(socketfd,message,strlen(message)) < 0){
				printTimeStamp(); cout<<"error writing to socket\n";
				return 0;
			...
			bzero(message,256);
			printTimeStamp(); cout<<"Enter the key to be deleted\n";
			cin>>message;
			if(write(socketfd,message,strlen(message)) < 0){
				printTimeStamp(); cout<<"error writing to socket\n";
				return 0;
			...
			printTimeStamp(); cout<<"Invalid action\n";
			continue;
		}
